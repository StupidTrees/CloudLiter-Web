...
#### 3. 查询所有发给自己的好友请求
- [ ] 完成
- 功能描述：根据用户id，在relationEvent表中找到发给该id，并且状态为“未接受”的所有行
...
#### 4. 计数所有发给自己的好友请求
- [ ] 完成
- 功能描述：根据用户id，在relationEvent表中计数发给该id，并且状态为“未接受”的所有行
...
#### 1. 新建relationEvent表
   ...
   | 字段 | 类型 | 含义 | 示例| 说明|
       ...
       | fromId| bigint| 用户1id|1| 链接到User表"id"字段的外键|
       |toId|bigint|用户2id|3|链接到User表"id"字段的外键|
       |state|enum|请求状态/类型|REQUESTING请求中<br>ACCEPTED被接受<br>REJECTED被拒绝<br>DELETE删好友|非空|
       |createdAt|date|创建时间|||
       |updatedAt|date|更新时间|||
