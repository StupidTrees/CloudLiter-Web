...
exports.responseFriendApply = async function resFriendApply(id, action) {
    if (equals(action, 'ACCEPT')) {
        ...
        try {
            let val = await convRepository.newFriendConversation(message.user1, message.user2)
...
exports.deleteFriend = async function (userId, friendId) {
    ...
    try {
        //同时删除对话词云
        value = await convRepository.getConversationId(userId, friendId)
        if (value) {
            await wordCloudRepository.deleteConversationWordCloud(userId, friendId)
            await eventRepository.deleteFriend(userId, value[0].get().id)
        } else {
