...
with tf.Graph().as_default():
    ...
    def face_insert():
        ...
        imagefile = request.form['imagePath']

        # opencv读取图片，开始进行人脸识别
        img = misc.imread(os.path.expanduser(imagefile), mode='RGB')
        # 设置默认插入时 detect_multiple_faces =Flase只检测图中的一张人脸，True则检测人脸中的多张
        # 一般入库时只检测一张人脸，查询时检测多张人脸
        imgUid = str(uuid.uuid1())
        images = image_array_align_data(img, imagefile, pnet, rnet, onet, detect_multiple_faces=True,saveDetectedFaces=True,imageId=imgUid)
        ...
        try:
            ...
            return json.dumps({'state': 'no_face'})

        f_, file_extension = os.path.splitext(imagefile)
        id_list = []
        # 存入数据库
        for j in range(0, len(emb_array)):
            face_mysql_instant = face_mysql.face_mysql()
            last_id = face_mysql_instant.insert_facejson(imgUid + "_" + str(j)+file_extension,
                                                         ",".join(str(li) for li in emb_array[j].tolist()), userId)
    ...
    def face_query():
        print("success in query")
        #获取查询条件  在ugroup中查找相似的人脸
        userId = request.form['userId']
        print(userId)
        imagefile=request.form['imagePath']
        print(imagefile)
        upload_files = Image.open(imagefile)
        rects = request.form['rects']
        print(rects)
        #获取post请求的图片到本地
        file = upload_files
        ...
        rectJsons = json.loads(rects)
        for i in range(0, len(rectJsons)):
            user_rects_dic = rectJsons[i]
            y_1=int(float(user_rects_dic["y"])*int(height))
            y_2=int(float(user_rects_dic["height"])*int(height))
            x_1=int(float(user_rects_dic["x"])*int(width))
            x_2=int(float(user_rects_dic["width"])*int(width))
            image_cropped = img[y_1:y_1 + y_2, x_1:x_1 + x_2]
            temp_images = image_array_align_data(image_cropped, image_path, pnet, rnet, onet)
            print(temp_images.shape)
            #判断如果如图没有检测到人脸则直接返回
            if len(temp_images.shape) < 4:
                continue
            feed_dict = {images_placeholder:temp_images, phase_train_placeholder: False}
            emb_array=[]
            try:
                emb_array = sess.run(embeddings, feed_dict=feed_dict)
        ...
        # pic_min_uid  是对应的用户id
            pic_min_scores, pic_min_names, pic_min_uid=[],[],[]
            #try:
            pic_min_scores, pic_min_names, pic_min_uid = face_query.get_socres(emb_array, userId)
        ...
        #如果提交的query没有group 则返回
            if len(pic_min_scores) == 0:
                data.append({'id':user_rects_dic["id"],'error': "not found user group"})
                continue
        ...
        #设置返回结果
            ...
            print(result)
            if len(result)==0 :
                data.append({'id':user_rects_dic["id"],"state":"success, but not match face"})
            else:
                data.append({'id':user_rects_dic["id"],"result":result})
...
def image_array_align_data(image_arr, image_path, pnet, rnet, onet, image_size=160, margin=32, gpu_memory_fraction=1.0,
                           detect_multiple_faces=True,saveDetectedFaces=False,imageId=""):
    ...
    if nrof_faces > 0:
        ...
        for i, det in enumerate(det_arr):
            ...
            if saveDetectedFaces:
                 # 保存检测的头像
                filename_base = './face_pic'
                filename = os.path.basename(image_path)
                _,file_extension = os.path.splitext(filename)
                # 多个人脸时，在picname后加_0 _1 _2 依次累加。
                output_filename_n = "{}/{}_{}{}".format(filename_base,imageId , i, file_extension)
